-- ============================================================
-- Migration 012 - Limiar relativo de manutenção + plano subsequente
--
-- Corrige a lógica de próxima manutenção: em vez de comparar a
-- leitura atual com o interval_value absoluto do plano, calcula
-- o limiar real como:
--   last_maintenance_reading + (next_plan.interval_value - last_plan.interval_value)
--
-- Isso garante que atrasos na revisão anterior "deslizem" o limiar
-- da próxima revisão para frente (ex: revisão de 500h feita às 510h
-- → próxima revisão de 1000h deve ocorrer às 1010h, não às 1000h).
--
-- Também expõe o plano subsequente (upcoming) para permitir o aviso
-- "você está próximo da revisão seguinte — considere antecipar o pedido".
--
...
select
  ...
  -- Próximo plano de manutenção (intervalo absoluto do plano)
  np.interval_value  as next_maintenance_interval,
  np.name            as next_maintenance_plan_name,

  -- Limiar real da próxima manutenção (ajustado pelo atraso acumulado)
  -- = leitura da última revisão + intervalo relativo até o próximo plano
  -- Para equipamento novo sem revisão: 0 + next_interval - 0 = next_interval
  case
    when np.interval_value is not null
    then coalesce(lm.reading_at_maintenance, 0)
         + np.interval_value
  ...
  case
    when up.interval_value is not null and np.interval_value is not null
    then coalesce(lm.reading_at_maintenance, 0)
...
-- (primeiro plano com interval_value maior que a leitura da última revisão)
left join lateral (
...
-- Plano subsequente (depois do próximo)
left join lateral (
  select interval_value, name
  from public.maintenance_plans
  where model_id = e.model_id
    and np.interval_value is not null
    and interval_value > np.interval_value
  order by interval_value asc
  limit 1
) up on true;
