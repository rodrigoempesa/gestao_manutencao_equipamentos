-- ============================================================
-- Migration 045 - Adiciona next_maintenance_plan_id à vw_equipment_status
-- ------------------------------------------------------------
-- Necessário para que o dashboard consiga pré-preencher o plano ao
-- abrir uma OS direto da linha do equipamento com status "vencido"
-- ou "atenção".
--
-- A coluna é adicionada no FIM do SELECT para que `create or replace
-- view` funcione sem precisar dropar a view (que tem dependentes).
-- ============================================================
...
select
  ...
  -- ⬇️ NOVO: id do próximo plano (para abrir OS já com a revisão pré-selecionada)
...
-- Próximo plano (mp.id adicionado para que np.id esteja disponível)
left join lateral (
  select * from (
    select
      mp.id,
